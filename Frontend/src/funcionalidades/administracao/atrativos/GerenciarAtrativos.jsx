            ...
            <ul aria-label="Atrativos ativos" className="grid gap-4 lg:grid-cols-2">
              {filtrados.map((atrativo) => (
                <li key={atrativo.id} className="flex min-w-0 flex-col rounded-2xl border border-floresta/10 bg-white p-5 shadow-sm [overflow-wrap:anywhere] sm:p-6">
                  ...
                  <Link to={`/admin/atrativos/${atrativo.id}/horarios`} aria-label={`Gerenciar horários: ${atrativo.nome}`} className={`mt-3 inline-flex min-h-12 items-center justify-center rounded-full border border-floresta px-4 py-2 font-semibold text-floresta hover:bg-creme ${foco}`}>Gerenciar horários</Link>
